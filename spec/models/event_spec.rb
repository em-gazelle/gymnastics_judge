...
RSpec.describe Event, type: :model do 
	...
	describe 'format_unmet_requirements' do
		subject { event.final_info }
		context 'no unmet reqs' do
			let(:event) { floor_met_reqs }
			it 'returns [], no errors' do
				expect(subject[:unmet_requirements]).to eq([])
			end
		end
		context 'Reqs unmet for beam/floor' do
			...
			it 'formats it prettily for easier HTML/JS parsing' do
				expect(subject[:unmet_requirements]).to eq(["1 A Skills", "2 B Skills", "3 C Skills", "Mount", "Dismount"]) 
			end
		end
		context 'unmet reqs for vault' do
			let(:event) { Fabricate(:event, event_name: "Vault") }
			it 'returns correct unmet reqs' do
				expect(subject[:unmet_requirements]).to eq(["Two Different Vaults"])
			end
		end
	end
